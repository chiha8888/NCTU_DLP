...
private:
    ...
    struct lookup {
        ...
        static const lookup& find(int row) {
            // an array with 2^16 lookup elements
...
public:
    ...
    int move_right() {
        ...
        uint64_t prev = raw;
        int score = 0;
        lookup::find(fetch(0)).move_right(move, score, 0); // an lookup object doing ->
...
public:
    ...
    feature(const feature& f) = delete;     // copy constructor is prohibited
    feature& operator =(const feature& f) = delete;
    virtual ~feature() { delete[] weight; }

    float& operator[] (size_t i) { return weight[i]; }  // can modify the return value weight[i]
    float operator[] (size_t i) const { return weight[i]; }  // can't modify the return value weight[i]
...
public:
    pattern(const std::vector<int>& p, int iso = 8) : feature(1 << (p.size() * 4)), iso_last(iso) { // get an weight array with 2^24 floating number
        if (p.empty()) {
         ...
         */
        // one pattern will have 8 isomorphic, and these 8 isomorphic pattern are sharing the same weight table
        //  weight table size: 16^6 * 4 bytes =  62.5 MB
        for (int i = 0; i < 8; i++) {
            board idx = 0xfedcba9876543210ull;
            if (i >= 4) idx.mirror();
            idx.rotate(i);
            for (int t : p) {
                isomorphic[i].push_back(idx.at(t));  // isomorphic[i]: 6*4 bit
            }
        }
    }
...
public:
    ...
    virtual float estimate(const board& b) const {
        ...
        for(int i=0;i<iso_last;i++){
            value+=(*this)[indexof(isomorphic[i],b)];  // 查 weight table
        }
        return value;
    }
...
public:
     ...
     * return whether the action is valid for the given state or not
     */
    bool assign(const board& b) {
        debug << "assign " << name() << std::endl << b;
        after = before = b;
        score = after.move(opcode); // how much score you get by this op action
        esti = score;
        return score != -1;
    }
...
public:
    ...
    float estimate(const board& b) const {
        ...
        float value = 0;
        for (feature* feat : feats) {   // 4feats*8isomorphic=32value
...
int main(int argc, const char* argv[]) {
    ...
    // initialize the features: 4 features, each includes 8 isomorphism
    ...
    // save score.csv
    save_scorecsv(scores,"../score.csv");
